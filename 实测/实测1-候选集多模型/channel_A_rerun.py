#!/usr/bin/env python3
"""后台补跑 channel-A 通道（今日限流严重，串行+长退避）。"""
...
for rep in (1, 2):
    for qid, prompt in Q.items():
        ...
        for attempt in range(5):
            try:
                body = json.dumps({"model": "channel-A", "messages": [{"role": "user", "content": prompt}],
                                   "max_tokens": 8000}).encode()
                req = urllib.request.Request("https://api.channel-A.com/v1/chat/completions", data=body,
                    headers={"Content-Type": "application/json",
                             "Authorization": f"Bearer {E['CHANNEL_A_API_KEY']}"}, method="POST")
                ...
                if txt.strip():
                    done.append({"ok": True, "text": txt, "rep": rep, "qid": qid, "model": "channel-A"})
